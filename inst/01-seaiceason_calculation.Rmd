...
for (i in seq_along(files)) {
  ## obj contains 2 rasters "adv" and "ret"
  obj <- calc_ice_season(files[i], threshval = 90)
  advance[[i]] <- obj$adv
  retreat[[i]] <- obj$ret
  daycount[[i]] <- calc(readAll(brick(files[i])) > 90, mean, na.rm = TRUE) * 365
...
saveRDS(advance, file.path(dp, "ice_90_advance.rds"))
saveRDS(retreat, file.path(dp, "ice_90_retreat.rds"))
saveRDS(daycount, file.path(dp, "ice_90_daycount.rds"))
